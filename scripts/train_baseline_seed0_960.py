#!/usr/bin/env python3
"""Run the D-Fire baseline at 960x960 for seed 0.

This is an independent resolution experiment, so it does not compare its
training hyperparameters with the existing 640px P2 run.  It still performs
the read-only data, weight, environment, and output-directory checks.

    python scripts/train_baseline_seed0_960.py --dry-run
    python scripts/train_baseline_seed0_960.py
"""
...
METHOD = "baseline"
SEED = 0
DEFAULT_TAG = "paper-v1_960"
ULTRA_REQUIRED = "8.4.42"

ARCHIVE = ROOT / "experiments" / "yolo11n" / "D-Fire_protocol_baseline_960_seed0"

CRITICAL_SOURCES = (
    "configs/protocol_960.yaml",
    "firesmoke/data.py",
...
def spec_for(tag: str):
    ...
    cfg = load_protocol(ROOT / "configs/protocol_960.yaml")
...
def preflight(cfg: dict, spec: dict) -> tuple[list[str], list[str]]:
    """Run read-only checks for the independent 960px experiment."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="960分辨率 baseline seed0 训练脚本")
    ...
    print(f"960分辨率 baseline 训练 —— seed={SEED} tag={spec['tag']}")
    print("  实验描述  : P3/P4/P5 baseline，960分辨率，独立分辨率实验")
